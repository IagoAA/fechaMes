...
BEGIN;  -- #INI :: IMPORTACAO UNIDADE REQUISITANTE >> ESTRUTURAORGANIZACIONAL
    /*
     ...
     - Unidades Requisitantes com mesma UA e mesmo LOCAL que a UL, não serão criadas
     - Estamos considerando somente as Unidades Requisitantes que não possuem correspondente com UL
    */
    INSERT INTO public.estruturaorganizacional
     (
      id
    ...
    BEGIN
      ...
      IF v_conta_id IS NOT NULL THEN
        ...
        VALUES 
            ...
            ,(now(), now(), 0, null, 'RELATORIO_PRIMEIRO_TITULO', 'Ministério do Planejamento Orçamento e Gestão - MP', 1)
            ,(now(), now(), 0, null, 'RELATORIO_SEGUNDO_TITULO', 'Diretoria de Administração - DIRAD', 1)
            ,(now(), now(), 0, null, 'RELATORIO_TERCEIRO_TITULO', 'Coordenação-Geral de Administração Predial - CGDAP', 1)
            ,(now(), now(), 0, null, 'MASCARA_NUMERO_IDENTIFICACAO', 'AAAA999999', 1);
      ELSE
        RAISE NOTICE 'ID conta_contabil_5292 NÃO ENCONTRADO!';
      END IF;
    END$$;
