...
BEGIN;  -- #INI :: MOVIMENTACAO DE DAS ENTRADAS >> ENTRADA
    ...
    */
    ...
    /*
     A estrat�gia usada para incluir os itens, foi agrup�-los por valor unit�rio para
     bater com o total, pois um dos itens do mesmo material pode ter valor unitario
     diferente por causa do resto de uma d�zima peri�dica.
    ...
    */
	...
	--Possivel erro aqui porque pode n�o ter a coluna codigo, rodar na m�o um por um caso de erro. 
    DO $$
    BEGIN
...
BEGIN;  -- #INI :: IMPORTACAO BAIXAS DE ESTOQUE >> BAIXA
    /*
      BAIXA ALMOXARIFADO
      ==================
      1. Somente uma baixa est� sem data da baixa, deixei ela de fora pois a data � obrigat�ria, por isso a codicao dt_baixa is not null;
...
BEGIN;  -- #INI :: IMPORTACAO DOA��ES DE MATERIAIS >> 
    --> FROM ASIWEB.AX_NOTA_REC NR
    --> JOIN ASIWEB.AX_NOTA_REC_ITEM NRI ON NRI.CD_NOTA_REC = NR.CD_NOTA_REC
COMMIT; -- #FIM :: IMPORTACAO DOA��ES DE MATERIAIS >> 
