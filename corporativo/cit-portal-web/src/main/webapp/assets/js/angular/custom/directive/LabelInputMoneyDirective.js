...
citApp.directive("labelInputMoney", ["$translate", function($translate) {
	...
	return {
		...
		template :  "<div class='form-group' ng-class=\"{'has-error': form[id].$error.required && (!form[id].$pristine || form.$submitted)}\">" +
					...
					" 	<input type='text' class='form-control text-{{textPosition}}' ng-change='eventoChange()' ng-keydown='removeModel($event)' id='{{id}}' name='{{id}}' ng-model='model' ng-disabled='disabled' maxlength='{{maxlength ? maxlength : 23}}' ng-required='obrigatorio' ui-money-mask='{{precisao}}'/>" +
					"</div>",
