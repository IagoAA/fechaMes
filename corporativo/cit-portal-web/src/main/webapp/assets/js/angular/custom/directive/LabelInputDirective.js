...
citApp.directive("labelInput", ["$translate", function($translate) {
	...
	return {
		...
		template :  "<div class='form-group' ng-class=\"{'has-error': (form[id].$error.required || form[id].$error.regexValidate) && (!form[id].$pristine || form.$submitted)}\">" +
					...
					"	</label>"+
					" 	<input type='{{type}}' ng-trim='{{trim}}' regex-validate='{{regex}}' ng-blur='eventoBlur()' ng-change='eventoChange()' class='form-control' id='{{id}}' maxlength='{{maxlength ? maxlength : 255}}' name='{{id}}' ng-model='model' ui-mask='{{mask}}' ng-disabled='disabled' ng-required='obrigatorio' placeholder='{{placeholder}}'/>" +
					"</div>",
