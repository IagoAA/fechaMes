    ...
    return { 
      ...
      template: '<li>' + 
        '<div class="node" data-node-id="{{ nodeId() }}">' +
          ...
          '<a ng-hide="selectedId" ng-href="#/assets/{{ nodeId() }}">{{ node.nome }}</a>' +
      ...
      link: function(scope, elm, attrs) {
        ...
        scope.toggleNode = function(nodeId) {
          ...
          if(!isVisible && scope.loadFn && childrenTree.length === 0) {
            ...
            elm.find("a.icon i").toggleClass("ace-icon fa fa-chevron-right");
            elm.find("a.icon i").toggleClass("ace-icon fa fa-chevron-down");
          }
        };
        ...
        if(scope.node.hasChildren) {
          elm.find("a.icon").append('<i class="ace-icon fa fa-chevron-right"></i>');
        }
