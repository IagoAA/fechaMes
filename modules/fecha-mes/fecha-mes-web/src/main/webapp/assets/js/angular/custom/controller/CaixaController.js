...
citApp.controller('CaixaController',['$scope', 'CaixaRepository', 'LuckRepository', '$translate', '$timeout', 'DominioRepository', 'EstruturaOrganizacionalRepository',
                                     function CaixaController($scope, CaixaRepository, LuckRepository, $translate, $timeout, DominioRepository, EstruturaOrganizacionalRepository) {
	$scope.caixa = {
		listaEstado : []
	};
	...
	DominioRepository.findAllDominio('tipoDocumentoCaixa').then(function(result) {
		$scope.dominiosTipoRestricao = result;
	});

	DominioRepository.findAllDominio('tipoRecolhimentoCaixa').then(function(result) {
		$scope.dominiosTipoRestricao = result;
	});

	//#################### INÍCIO MÉTODOS AUTOCOMPLETE ESTRUTURA DESTINO ####################
	// Método responsável por listar lojas através do nome digitado e a organização do usuário logado
	$scope.findLoja =  function(value) {

		return EstruturaOrganizacionalRepository.listarEstruturasOrganizacionaisLojaPorNomeEOrganizacao(value, $scope.usuarioLogado.organizacao.id).then(function(result) {
		   return result;
	   });
	};

	// Método responsável por limpar o auto complete loja
	$scope.limparAutoCompleteLoja = function(value) {

		value = {};
		$scope.caixa.loja = null;
	};

	// Método responsável por setar a loja selecionada
	$scope.setLoja = function(value){

		if(value && value.id !== undefined){

			$scope.caixa.loja = value;
		}
	};
	//#################### FIM MÉTODOS AUTOCOMPLETE ESTRUTURA DESTINO ####################
