citApp.directive("componenteAssociarDocumentos", ["$translate", "FileUploader", "DominioRepository", "ParceiroRepository", "DocumentoRepository", "$modal", "$filter", "$timeout", "AnexoRepository", function($translate, FileUploader, DominioRepository, ParceiroRepository, DocumentoRepository, $modal, $filter, $timeout, AnexoRepository) {
	return {
		scope : {
		...
		controller : ['$scope', function($scope){
			...
			if($scope.labelAlertTooltip === undefined) {
				$scope.labelAlertTooltipCopy = $translate.instant('LABEL.ASSOCIAR_DOCUMENTOS') + ' ' + $translate.instant('LABEL.CAMPO_OBRIGATORIO');
			}
		...
		link : function($scope, $element, attributes) {
			...
			$scope.inicializarModalDocumentos = function(){
				...
				$scope.openModal('modal-documentos.html');
			};
		    	...
		    	formDialogDocumentos.$submitted = true;
		 		if(formDialogDocumentos.$valid && !($scope.documento.dataEmissao.getTime() > hoje)){
		 			...
		 			$scope.documentos.forEach(function (documento, key) {
						if(documento.$uuid === $scope.documento.$uuid){
		 			...
		 			$scope.$parent.showAlert('error', $translate.instant('VALIDACAO.DATA_EMISSAO_DATA_ATUAL'));
